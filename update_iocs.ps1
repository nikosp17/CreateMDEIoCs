...
function identifyIndicator([string]$indicator) {
    ...
    if ($indicator -match $sha1HashPattern) {
            return 'FileSha1,' + $line + ',"",BlockAndRemediate,High,"A malicious file has been detected in one of the endpoints.","Malicious File","Inspect the malicious file hash (SHA-1) in the context of the environment in which it has appeared (user, workstatiion)","","Execution","",true'
    }
    elseif ($indicator -match $sha256HashPattern) {
        return 'FileSha256,' + $line + ',"",BlockAndRemediate,High,"A malicious file has been detected in one of the endpoints.","Malicious File","Inspect the malicious file hash (SHA-256) in the context of the environment in which it has appeared (user, workstatiion)","","Execution","",true'
    }
    elseif ($indicator -match $ipAddressPattern) {
        return 'IpAddress,' + $line + ',"",Block,High,"Nefarious IP.","IP connected to malicious activity","Inspect the malicious IP in the context of the environment in which it has appeared (user, workstatiion, resource)","","SuspiciousActivity","",true'
    }
    elseif ($indicator -match $urlPattern) {
        return 'Url,' + $line + ',"",Block,High,"Custom Domain Name Block.","Malicious Domain","Inspect the malicious URL in the context of the environment in which it has appeared (user, workstatiion)","","SuspiciousActivity","",true'
    }
    elseif ($indicator -match $domainPattern) {
        return 'DomainName,' + $line + ',"",Block,High,"Custom Domain Name Block.","Malicious Domain","Inspect the malicious domain in the context of the environment in which it has appeared (user, workstatiion)","","SuspiciousActivity","",true'
    }
    else {
